...
"""
   ...
   Modified by Cyb3rc0d3 for Python 3 when using for AWAE etc
   v0.4
   -Modified to run in Python 3
   -Changed URLLIB for Python 3
   -Fixed connection error to HTML Purifier by using HTTPS and ignore cert warning
...
XSSURL        = "https://htmlpurifier.org/live/smoketests/xssAttacks.xml"
...
def fetchXML():
    ...
    response = urllib.urlopen(XSSURL, context=context)
